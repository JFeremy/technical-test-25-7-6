"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ServicesModule = void 0;
const common_1 = require("@nestjs/common");
const lawn_service_1 = require("./lawn/lawn.service");
const mower_service_1 = require("./mower/mower.service");
const lawn_context_service_1 = require("./lawn-context/lawn-context.service");
const upload_file_service_1 = require("./upload-file/upload-file.service");
let ServicesModule = class ServicesModule {
};
exports.ServicesModule = ServicesModule;
exports.ServicesModule = ServicesModule = __decorate([
    (0, common_1.Module)({
        providers: [lawn_service_1.LawnService, mower_service_1.MowerService, lawn_context_service_1.LawnContextService, upload_file_service_1.UploadFileService],
        exports: [
            lawn_context_service_1.LawnContextService,
            lawn_service_1.LawnService,
            mower_service_1.MowerService,
            lawn_context_service_1.LawnContextService,
            upload_file_service_1.UploadFileService,
        ],
    })
], ServicesModule);
//# sourceMappingURL=services.module.js.map